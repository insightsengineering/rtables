...
#' @param varnames character vector. Names for \code{vars} which will appear in pathing. When \code{vars} are all unique
#'   this will be the variable names. If not, these will be variable names with suffixes as necessary to enforce
#'   uniqueness.
#' @param split_format FormatSpec. Default format associated with the split being created.
#' @param split_label string. Label string to be associated with the table generated by the split. Not to be confused
#'   with labels assigned to each child (which are based on the data and type of split during tabulation).
#' @param nested boolean, Add this as a new top-level split  (defining a new subtable directly under root). Defaults to
#'   \code{FALSE}
#' @param format FormatSpec. Format associated with this split. Formats can be declared via strings (\code{"xx.x"}) or
#'   function. In cases such as \code{analyze} calls, they can character vectors or lists of functions.
#' @param cfun list/function/NULL. tabulation function(s) for creating content rows. Must accept \code{x} or \code{df}
#'   as first parameter. Must accept \code{labelstr} as the second argument. Can optionally accept all optional
...
#' @param afun function. Analysis function, must take \code{x} or \code{df} as its first parameter. Can optionally take
#'   other parameters which will be populated by the tabulation framework. See Details in \code{\link{analyze}}.
#' @param inclNAs boolean. Should observations with NA in the \code{var} variable(s) be included when performing this
#'   analysis. Defaults to \code{FALSE}
...
#' @param compfun function/string. The comparison function which accepts the analysis function outputs for two different
#'   partitions and returns a single value. Defaults to subtraction. If a string, taken as the name of a function.
#' @param label_fstr string. An sprintf style format string containing. For non-comparison splits, it can contain  up to
#'   one \code{"\%s"} which takes the current split value and generates the row/column label. Comparison-based splits it
#'   can contain up to two \code{"\%s"}.
#' @param child_labels string. One of \code{"default"}, \code{"visible"}, \code{"hidden"}. What should the display
#'   behavior be for the  labels (i.e. label rows) of the children of this split. Defaults to \code{"default"} which flags
#'   the label row as visible only if the child has 0 content rows.
#' @param extra_args list. Extra arguments to be passed to the tabulation function. Element position in the list
#'   corresponds to the children of this split. Named elements in the child-specific lists are ignored if they do not
#'   match a formal argument of the tabulation function.
#' @param name character(1). Name of the split/table/row being created. Defaults to same as the corresponding label,
#'   but is not required to be.
#' @param cuts numeric. Cuts to use
#' @param cutlabels character (or NULL). Labels for the cuts
#' @param cutlabelfun function. Function which returns either labels for the cuts or NULL when passed the return value
#'   of \code{cutfun}
#' @param cumulative logical. Should the cuts be treated as cumulative. Defaults to \code{FALSE}
#' @param cutfun function. Function which accepts the \emph{full vector} of \code{var} values and returns cut points to
#'   be used (via \code{cut}) when splitting data during tabulation
#' @param indent_mod numeric. Modifier for the default indent position for the structure created by this
...
#' @param show_labels character(1). Should the variable labels for corresponding to the variable(s) in \code{vars} be
#'   visible in the resulting table.
#' @param table_names character. Names for the tables representing each atomic analysis. Defaults to \code{var}. This is
#'   the name that appears in the row paths, i.e. see `row_paths_summary`. This is sometimes useful if the same
...
#' @param FUN function. Tabulation function. Will be passed subsets of \code{x} defined by the combination of \code{col_by} and \code{row_by} and returns corresponding cell value
