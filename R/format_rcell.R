...
format_rcell <- function(x, format, output = c("ascii", "html")) {
  ...
  txt <- if (is.null(format)) {
             ...
             switch(
                 ...
                 "xx. / xx." = paste(lapply(x, round, 0), collapse = " / "),
                 "xx.x / xx.x" = paste(lapply(x, round, 1), collapse = " / "),
                 "xx.xx / xx.xx" = paste(lapply(x, round, 2), collapse = " / "),
                 "xx.xxx / xx.xxx" = paste(lapply(x, round, 3), collapse = " / "),
                 ...
                 "xx. (xx.%)" = paste0(round(x[1],0), " (", round(x[2]*100, 0), "%)"),
