...
#' These functions can be used to create or add to a split result in functions which
#' implement core splitting or post-processing within a custom split function.
#'
#' @param values character or list<SplitValue>. The values associated with each facet
#' @param datasplit list<data.frame>. The facet data for each facet generated in the split
#' @param labels character. The labels associated with each facet
#' @param extras NULL or list. Extra values associated with each of the facets which will be passed to analysis functions applied within the facet.
#'
...
#'
#' @examples
#' splres <- make_split_result(values = c("hi", "lo"), datasplit = list(hi = mtcars, lo = mtcars[1:10,]), labels = c("more data", "less data"))
#'
#' splres2 <- add_to_split_result(splres, values = "med", datasplit = list(med = mtcars[1:20,]), labels = "kinda some data")
...
#' addition  to  `spl`,  and   `fulldf`,  and  can  optionally  accept
#' `.spl_context`. They must each return a modified version
