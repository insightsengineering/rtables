...
do_label_row <- function(rdfrow, maxlen) {
  pth <- rdfrow$path[[1]]
  c(as.list(pth), replicate(maxlen - length(pth), list(NA_character_)), list(row_num = rdfrow$abs_rownumber, content = FALSE, node_class = rdfrow$node_class)) # nolint
}
...
## #' @param paginate logical(1). Should \code{tt} be paginated before writing the file. Defaults to `TRUE` if any sort of page dimension is specified. # nolint
...
tt_to_flextable <- function(tt, paginate = FALSE, lpp = NULL,
  ...
  if (paginate) {
    ...
    return(mapply(tt_to_flextable,
      tt = tabs, colwidths = cinds, MoreArgs = list(paginate = FALSE, total_width = total_width),
      SIMPLIFY = FALSE
    ))
  }
  ...
  if (length(all_footers(tt)) > 0) {
    flx <- flextable::hline(flx,
      i = length(matform$ref_footnotes),
...
export_as_pdf <- function(tt,
  ...
  list(file = file, npages = npages, exceeds_width = exceeds_width, exceeds_height = exceeds_height, lpp = lpp, cpp = cpp) # nolint
}
