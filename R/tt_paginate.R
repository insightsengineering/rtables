...
setMethod(
  ...
  function(x, colwidths, max_width, fontspec, col_gap = 3) {
    ...
    }, s = c(obj_label(x), fcells), w = (colwidths %||% max_width) %||% vector("list", length(c(obj_label(x), fcells))), SIMPLIFY = FALSE)))
...
setMethod(
  ...
  function(x, colwidths, max_width, fontspec, col_gap = 3) {
    ...
    main_nls <- vapply(
      seq_along(h_rows),
      function(i) max(nlines(h_rows[[i]], colwidths = colwidths, fontspec = fontspec, col_gap = col_gap), nlines(tl[i], colwidths = colwidths[1], fontspec = fontspec)),
      1L
    )
...
setMethod(
  ...
  function(tt, colwidths = NULL, visible_only = TRUE,
    ...
    reflines <- sum(sapply(c(rrefs, crefs), nlines, colwidths = colwidths, max_width = max_width, fontspec = fontspec, col_gap = col_gap)) ## col_gap not strictly necessary as these aren't rows, but why not
    ret <- pagdfrow(
      row = tt,
      rnum = rownum,
      colwidths = colwidths,
...
paginate_table <- function(tt,
  ...
  if (is.null(colwidths)) {
    colwidths <- propose_column_widths(matrix_form(tt, indent_rownames = TRUE, fontspec = fontspec, col_gap = col_gap), fontspec = fontspec)
  }
