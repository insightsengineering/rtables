...
## TODO XXX @wadella figure out what to do with the cod ehere. Vignette (needs tern dep removed first) or what?
...
##             lyt = split_cols_by(lyt, cb, label = cb, ref_group = ref_groups[[i]])
##         else
##             lyt = split_cols_by(lyt, cb, label = cb)
##     }
##     lyt
## }
...
##         analyze("rsp", TF_counter( nms = c("Responders", "Non-Responders")),
##                 var_label = " ", format = "xx (xx.%)") %>%
        
##         ## response rate CI
##         analyze("rsp", var_label = " ",
##                          afun = function(x) {
...
##         split_rows_by("AVALC", " ",
##                             split_fun = reord_levs_sfun(neworder = c("CR", "PR", "SD", "NON CR/PD", "PD", "NE"), drlevels = TRUE), nested = TRUE) %>%
##         ## counts are a summary content row
##         summarize_row_groups("AVALC", "%s") %>%
##         ## CIs are an analysis row
##         add_analyzed_var("AVALC", var_label = " ", afun = function(x, .N_col) {
