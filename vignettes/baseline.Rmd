...
basic_table() %>%
  ...
  analyze("AGE", afun = function(x, .ref_group) {
    in_rows(
      "Average Difference" = rcell(mean(x) - mean(.ref_group), format = "xx.xx")
    )
  }) %>%
  build_table(DM)
```
...
basic_table() %>%
  ...
  analyze("AGE", afun = function(x, .ref_group, .in_ref_col){
    if (.in_ref_col) {
      in_rows("Average Difference" = rcell(""))
    } else {
      in_rows("Average Difference" = rcell(mean(x) - mean(.ref_group), format = "xx.xx"))
    }
  }) %>%
  build_table(DM)
```
