...
We now have to the add a count table of `AEDECOD` for each `AEBODSYS`. The defaulty `analyze` behaviour for a factor is to create the count table per level (using `rtab_inner`):
...
To illustrate custom analyses function we will now rectreate this behavior.
...
basic_table() %>% 
  ...
  analyze("AETOXGR", indent_mod = -1, show_labels = "hidden") %>%
  build_table(ADAE2)
```
