...
And next we will add the Multinomial response table. We already 

So basically we are doing a row-split by response level, and then doing the same thing as we did for the binary response table above. However, when we want to do row-split, that means we need to also take into account the `.N_col` as total and then reconstruct the full logical vector.

```{r}
s_prop <- function(df, labelstr, .N_col) {
  in_rows(
    rcell(nrow(df) * c(1, 1/.N_col), format = "xx (xx.xx%)"),
    rcell(binom.test(nrow(df), .N_col)$conf.int * 100, format = "(xx.xx, xx.xx)"),
    .labels = c(labelstr, "95% CI (Wald, with correction)")
  )
}
...
basic_table() %>%
  ...
  split_rows_by(
    var = "AVALC",
    splfun = reorder_split_levels(neworder = c("CR", "PR", "SD", "NON CR/PD", "PD", "NE"), drlevels = TRUE), 
    nested = FALSE
  ) %>%
  summarize_row_groups(cfun = s_prop) %>%
  build_table(ADRS_BESRSPI)
```
...
basic_table() %>%
  ...
  summarize_row_groups(cfun = function(df, labelstr, .N_col) {
    res <- s_prop(df, rsp_label(labelstr), .N_col)
    
    if (labelstr == "NE") res[-2] else  res
