...
Note that the `UNDIFFERENTIATED` and `U` level still show up in the table. This is because tabulation respects the factor levels and level order, exactly as the `split` and `table` function. If empty levels should be dropped then the `split_*` functions need to know this via the  `split_fun` argument which excepts a splitting function, i.e. `drop_split_levels` here. Splitting is a bigger topic and we will address this in another vignette in more details. 
...
If we only want to stratify by gender for the `AGE` analysis one can do this using the `nested` argument:
