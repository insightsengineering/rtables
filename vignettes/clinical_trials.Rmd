...
basic_table() %>% 
  ...
  analyze("AETOXGR", afun = function(x) {
    in_rows(
      "--Any Grade--" = sum(!is.na(x)),
      .list = as.list(table(x))
    )
  }, var_labels = "--All AEBODSYS--", show_labels = "visible") %>%
  split_rows_by("AEBODSYS", child_labels = "visible", nested = FALSE) %>%
  split_rows_by("AEDECOD", split_fun = add_overall_level("--All AEDECOD--", trim = TRUE), child_labels = "visible")  %>%
  summarize_row_groups(label_fstr = "--Any Grade--") %>%
  analyze("AETOXGR", table_count_once_per_id, indent_mod = -1, show_labels = "hidden") %>%
  build_table(ADAE2)
```
