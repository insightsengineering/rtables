...
Note that all the data is created using random number generators. All `ex_*` data which is currently attached to the `rtables` package is stored in the [`formatters`](https://insightsengineering.github.io/formatters/) package and was created using the [`random.cdisc.data`](https://insightsengineering.github.io/random.cdisc.data/) R package.
...
For the next table variation we only stratify by gender for the `AGE` analysis. To do this the `nested` argument has to be set to `FALSE` in the `analyze` call:
