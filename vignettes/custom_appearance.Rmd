...
Similarly, wrapping can be applied when exporting a table via one of the four `formatters::export_as_*` functions or the `export_as_tsv()` function, and when implementing pagination via the `paginate_table()` function from the `rtables` package. In these cases, the rendered column widths are set using the `colwidths` argument which takes input in the same format as the `widths` argument of `toString()`.
