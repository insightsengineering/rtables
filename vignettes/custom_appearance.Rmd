...
The sequence of sequence of strings printed in the area between the column header display and the first row label (\emph{top left material}) can be modified using the `top_left()` function.
...
basic_table(
  ...
  split_cols_by("ARM") %>%
  split_rows_by("SEX", page_by = TRUE, page_prefix = "Patient Subset - Gender", split_fun = drop_split_levels, indent_mod = 3) %>%
...
With split instructions, label visibility of the groups can be modified. `visible`, `hidden` and `topleft`.For analyze calls, \code{default} indicates that the variable should be visible if and only if
multiple variables are analyzed at the same level of nesting.

Location of the variable label can be modified to be `visible`, `hidden` or present on the `topleft` for analyze level splits.
...
basic_table(
  ...
  split_cols_by("ARM") %>%
  split_rows_by("SEX", page_by = TRUE, page_prefix = "Patient Subset - Gender", split_fun = drop_split_levels, indent_mod = 3, child_labels = "hidden") %>%
