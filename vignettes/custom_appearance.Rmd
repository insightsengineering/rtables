...
`dec_right`, and `dec_left` for decimal alignments.

Please consider using `?in_rows` and `?rcell` for further
clarifications on the two arguments, and use `rtables_aligns()` to see
...
1. separating title + subtitles from the column labels + topleft
   materials,
...
labels of any subgroups should be hidden, the `label_pos` arugment
