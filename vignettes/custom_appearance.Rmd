...
The sequence of strings printed in the area between the column header display and the first row label \emph{top left material} can be modified during pre-processing within the table constructor `basic_table()` using `append_topleft`, the label position in row splits `split_rows_by` and during post-processing using the `top_left()` function. Note: No structure or indenting is applied to these strings.

Within the table construction:
...
Table title, table body, referential footnotes and and main footers can be inset from the left alignment of the titles and provenance footer materials. This can be modified within the table constructor `basic_table()` or during post-processing with `table_inset()`.

Using the table constructor:
...
Tables by default do not have an indent at the split level. A custom indent value can be supplied with the `indent_mod` argument within a split function.
...
Visibility of labels for the groups generated by a split can also be modified using the `child_label` argument with a split call.
```{r}
...
build_table(lyt, DM) # notice the placement of the "AGE" label.
...
build_table(lyt2, DM) # note that the levels associated with the variable "SEX" no longer appear.
```
...
basic_table(
  ...
  split_cols_by("ARM") %>%
  split_rows_by("SEX", page_by = TRUE, page_prefix = "Patient Subset - Gender", split_fun = drop_split_levels, label_pos = "hidden") %>%
