...
We can declare one of the columns in our table layout the "baseline" or "control" column, by using `split_cols_by()` instead of `split_cols_by()`. Once this is done, we can declare comparisons against that group as part of the layout.

Setup
```
collyt2 = NULL %>% split_cols_by("ARM", "Arm A", lbl = "Arm") %>%
...
We do this by calling `analyze_against_baseline`
...
```
layout = collyt2 %>%
       analyze_against_baseline("AGE", afun = mean)
```
...
For convenience we provide a helper function for comparisons which based on the 2xk table of (baseline vs column) against (levels of the variable): `analyze_against_baseline_2dtable()`.
...
```
layout = collyt2 %>%
       analyze_against_baseline_2dtable(var = "AGE",
...
```
layout = collyt2 %>%
       analyze_against_baseline("AGE", afun = identity,
...
layout = collyt2 %>%
       ...
       summarise_row_groups_count("RACE", lblfmt = "%s (n)")
```
