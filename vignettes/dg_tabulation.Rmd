...
`create_colinfo` is in `make_subset_expr.R`. Here, we see that if `topleft` is present in `build_table`, it will override the one in `lyt`. 

xxx -> add a warning for NAs in the splits 

xxx -> precedence for col counts: document it if not present. Original use case is that you split events while the right colcounts is patients. First only counts as vector was added, but it is often the case that you have the possibility to add alt_counts_df
