...
a_cox_summary <- function(df,
  ...
  ## Use tern function to calculate and extract information relevant to cov from
  ## the model. Default controls are applied via the `control_coxreg` function from tern
  cov_rows <- tern:::h_coxreg_extract_interaction(
    effect = "ARM", covar = cov, mod = model, data = df,
    control = tern::control_coxreg(), at = NULL
  )
