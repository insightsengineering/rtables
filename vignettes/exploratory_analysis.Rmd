...
specifying a data.frame with variable(s) to tabulate. The first variable
controls how to split the data by rows. Optionally, a second variable
can be used to control how to define the column facets of the table.
...
An additional note is that any variables used as the row or column
facets should not have any empty strings (""). This is because non empty
values are required as labels when generating the table. The code below
...
So far all the examples we have seen used counts to summarize the data
in each table cell as this is the default analysis performed by
`qtable()`. Internally, a single variable specified by `avar` is used to
generate the counts in the table. The default is the first variable in
`data` which is "STUDYID" in the case of `ex_adsl`.
...
The resulting table is empty because all the values of the analysis
variable are `NA`. 

Remember to always pre-process the data for `NA`
values when doing quick exploratory analysis and using the default
counts aggregate function to avoid misleading results.
...
Explicitly labelling the `NA` levels in the column facet adds a column to the
table:
```{r}
...
`STRATA1` and `STRATA2`. The number 18 in the second row summarizes the
count of records part of `ARM` level "A: Drug X", `STRATA1` level "A",
and `STRATA2` level "S1". The percent is calculated as the cell count
divided by the column count as shown in the table header. So we can see
that the mean `AGE` of 31.61 in that subgroup is based on 18 subjects
...
-   check summarize_groups interaction with row_labels
