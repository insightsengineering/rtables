...
* cell, row, column, table reference system (**TODO: is currently missing**)
* path based access to cell content which will be useful for automated content generation **TODO: work in progress**
...
Note that we use factors so that the level order is represented in the row or column order in the tabulations that will follow.
...
Please take a moment and think about the information contained in this table:
...
In `rtables`basic table is defined to have one column representing all data and 0 rows. Analyzing a variable is one way of adding a row:
...
In the code above we first described the table and assigned that description to a variable `l`. We then built the table using actual data with `build_table`. The description of a table is called a table layout. `basic_table` is the start of every table layout and contains the information that we have one column representing all data. The `analyze` instruction adds to the layout that the `age` variable should be analyzed with the `mean` analysis function and the result should be rounded to `1` decimal place.

Hence, a layout is "pre-data", that is, we do not need any data to describe how to build a table. Data is only needed when the actual table is constructed with `build_table`. We can look at the layout isolated: 
...
using those functions it is possible to create a wide variety of tables as we will show in this and the other vignettes in this package.
...
The resulting table has one column per factor level of `arm`. So the data represented by the first column is `df[df$arm == "ARM A", ]`. Hence, the columns partition the data which is also described in the `split_cols_by` layout function. 
...
So far, we have created layouts with analysis instructions (`analyze`) and column splitting (`split_cols_by`). This resulted with a table that has multiple columns and one data row. We will add more row structure by stratifying the mean analysis by country (i.e. adding a split in the row space):
...
In clinical trials analyses the number of patients per column is often referred to as `N` (rather than the overall population which is commonly referred to as `N`). In `rtables` the column counts can be added using the `add_colcounts` instruction:
