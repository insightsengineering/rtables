...
There are a number of other table frameworks available in `R` such as [gt](https://gt.rstudio.com/) from RStudio, [xtable](https://CRAN.R-project.org/package=xtable), [tableone](https://CRAN.R-project.org/package=tableone), [huxtable](https://CRAN.R-project.org/package=huxtable) , and [tables](https://CRAN.R-project.org/package=tables) to name a few. One reason to implement `rtables` was that to report clinical trials data the tables are often outputted in ASCII and paginated, which can be challenging to with the table packages just mentioned. `rtables` has a strong focus on tabulation, organizing and accessing information where other packages such as `gt` do a good job with  providing functionality to markup and output tables (in html and markdown).

In the remainder of this vignette we give a short introduction into `rtables` by tabulating a fairly complex table. The content is based on the useR 2020 presentation from Gabriel Becker.
