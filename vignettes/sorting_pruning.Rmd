...
be threated differently in the scoring function as they are retrieved also differently. For example, the internally defined `cont_n_allcols` will work with calling content value

Another difference between pruning and sorting is that sorting occurs at particular places in the table, as defined by a path. The path can contain "*" to indicate that at that portion of the structure sorting should occur **separately** within the branch of the path.
...
Sort the ethnicity by observations in increasing order:
...
silly_lyt <- basic_table() %>%
    split_rows_by("cyl") %>%
    analyze("mpg", silly_afun)

silly_tbl <- build_table(silly_lyt, mtcars)
silly_tbl
```

The path required to sort the rows inside our "analysis" of `mpg` is then `c("cyl", "*", "mpg")`:

```{r}
scorefun <- function(tt) { mean(unlist(row_values(tt)))}
sort_at_path(silly_tbl, c("cyl", "*", "mpg"), scorefun)
```
